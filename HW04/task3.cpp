    ...
    for(int i=0; i<N; i++){
      for(int j=0; j<N; j++){
        if(i!=j){
          ...
          dy = pos[j][1]-pos[i][1];
          dz = pos[j][2]-pos[i][2];
          double temp = dx*dx + dy*dy + dz*dz + softening*softening;
          inv_r3 = 1.0/std::sqrt(temp*temp*temp);
          #pragma omp atomic
          acc[i][0] +=G*(dx*inv_r3) * mass[j];
          #pragma omp atomic
          acc[i][1] +=G*(dx*inv_r3) * mass[j];
          #pragma omp atomic
          acc[i][2] +=G*(dx*inv_r3) * mass[j];
        }
      }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    for (int step = 0; step < Nt; step++) {
        ...
        // For debug: save positions to CSV at each step (optional)
